...
vec4 light_cache_get_level(const uint level, const vec3 pos, const vec3 normal) {
...
vec4 light_cache_get(const vec3 pos, const vec3 normal) {
    const uint level = lc_level_for_pos(pos);
    return light_cache_get_level(level, pos, normal);
}
...
void light_cache_update(const vec3 pos, const vec3 normal, const vec3 irr) {
    ...
    if (vtx.hash != hash2_grid_level(grid_idx, level)
        ...
        vtx.irr_N = light_cache_get_level(level + 1, pos, normal);
