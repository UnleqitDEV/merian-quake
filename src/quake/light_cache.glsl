...
#define LIGHT_CACHE_TAN_ALPHA_HALF 0.005
...
uint lc_level_for_pos(const vec3 pos) {
    ...
    return uint(LIGHT_CACHE_LEVELS * pow((target_grid_width - LIGHT_CACHE_MIN_GRID_WIDTH) / (LIGHT_CACHE_MAX_GRID_WIDTH - LIGHT_CACHE_MIN_GRID_WIDTH), 1 / 9.));
}
