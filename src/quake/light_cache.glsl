...
ivec3 lc_grid_idx_for_level_interpolate(const uint level, const vec3 pos, const vec3 normal, inout uint rng_state) {
...
ivec3 lc_grid_idx_for_level_closest(const uint level, const vec3 pos, const vec3 normal, inout uint rng_state) {
    float grid_width = cell_width_for_level_poly(level, LIGHT_CACHE_MAX_LEVEL, LIGHT_CACHE_MIN_GRID_WIDTH, LIGHT_CACHE_MAX_GRID_WIDTH, 8);
...
vec4 light_cache_get(const vec3 pos, const vec3 normal, inout uint rng_state) {
    ...
    for (int level = 0; level <= LIGHT_CACHE_MAX_LEVEL; level++) {
        const ivec3 grid_idx = lc_grid_idx_for_level_interpolate(level, pos, normal, rng_state);
